"""Phase 9 evaluation - Phase 5B vs Phase 7 vs Phase 8 vs Phase 9 Comparison.
...
OUT_ROOT = "outputs/phase9_targeted"

def add_records(records, condition, severity, sample_indices, input_paths, target_paths, p5b_out, p7_out, p8_out, p9_out, target, lpips_model, decomposition):
    for i, sample_index in enumerate(sample_indices):
        ...
        for model_name, output in (("phase5b", p5b_out[i : i + 1]), ("phase7", p7_out[i : i + 1]), ("phase8", p8_out[i : i + 1]), ("phase9", p9_out[i : i + 1])):
...
def make_summary(df):
    ...
    for (condition, severity), group in df.groupby(["condition", "severity"], sort=False):
        ...
        win_rates[f"{condition}|{severity}"] = {
            # Phase 9 vs Phase 5B
            "p9_vs_p5b": {
                "psnr": float((pivot["psnr"]["phase9"] > pivot["psnr"]["phase5b"]).mean() * 100),
                "ssim": float((pivot["ssim"]["phase9"] > pivot["ssim"]["phase5b"]).mean() * 100),
                "lpips": float((pivot["lpips"]["phase9"] < pivot["lpips"]["phase5b"]).mean() * 100),
                "mae": float((pivot["mae"]["phase9"] < pivot["mae"]["phase5b"]).mean() * 100),
                "hf_err": float((pivot["hf_err"]["phase9"] < pivot["hf_err"]["phase5b"]).mean() * 100),
            },
            # Phase 9 vs Phase 7
            "p9_vs_p7": {
                "psnr": float((pivot["psnr"]["phase9"] > pivot["psnr"]["phase7"]).mean() * 100),
                "ssim": float((pivot["ssim"]["phase9"] > pivot["ssim"]["phase7"]).mean() * 100),
                "lpips": float((pivot["lpips"]["phase9"] < pivot["lpips"]["phase7"]).mean() * 100),
                "mae": float((pivot["mae"]["phase9"] < pivot["mae"]["phase7"]).mean() * 100),
                "hf_err": float((pivot["hf_err"]["phase9"] < pivot["hf_err"]["phase7"]).mean() * 100),
            },
            # Phase 9 vs Phase 8
            "p9_vs_p8": {
                "psnr": float((pivot["psnr"]["phase9"] > pivot["psnr"]["phase8"]).mean() * 100),
                "ssim": float((pivot["ssim"]["phase9"] > pivot["ssim"]["phase8"]).mean() * 100),
                "lpips": float((pivot["lpips"]["phase9"] < pivot["lpips"]["phase8"]).mean() * 100),
                "mae": float((pivot["mae"]["phase9"] < pivot["mae"]["phase8"]).mean() * 100),
                "hf_err": float((pivot["hf_err"]["phase9"] < pivot["hf_err"]["phase8"]).mean() * 100),
            }
        }
...
def build_verdict(summary_df, baseline_df):
    ...
    # Threshold for material regression on original validation compared to Phase 8:
    # PSNR within 0.015 dB of Phase 8, SSIM within 0.0015, LPIPS within 0.0025
    orig_psnr_ok = p9_base.psnr >= p8_base.psnr - 0.015
    orig_ssim_ok = p9_base.ssim >= p8_base.ssim - 0.0015
    orig_lpips_ok = p9_base.lpips <= p8_base.lpips + 0.0025
    orig_mae_ok = p9_base.mae <= p8_base.mae + 0.0003
    orig_hf_ok = p9_base.hf_err <= p8_base.hf_err + 0.00008

    orig_preserved = orig_psnr_ok and orig_ssim_ok and orig_lpips_ok and orig_mae_ok and orig_hf_ok

    # Robustness check:
    # Check Gaussian blur medium/high and noise+blur medium/high
    ...
    combo_h_p8 = summary_df[(summary_df.model == "phase8") & (summary_df.condition == "noise_plus_blur") & (summary_df.severity == "high")].iloc[0]

    # Robustness is improved if it shows a gain in PSNR over Phase 8 on targeted stress tasks
    robustness_improved = (blur_m_p9.psnr_mean > blur_m_p8.psnr_mean + 0.01) or (blur_h_p9.psnr_mean > blur_h_p8.psnr_mean + 0.01) or (combo_h_p9.psnr_mean > combo_h_p8.psnr_mean + 0.01)

    if orig_preserved and robustness_improved:
        verdict = "CASE A — NEW CHAMPION"
    elif robustness_improved:
        avg_gain = ((blur_m_p9.psnr_mean - blur_m_p8.psnr_mean) + (blur_h_p9.psnr_mean - blur_h_p8.psnr_mean) + (combo_h_p9.psnr_mean - combo_h_p8.psnr_mean)) / 3.0
        if avg_gain >= 0.08:
            verdict = "CASE D — ROBUSTNESS SPECIALIST"
        else:
            verdict = "CASE B — IMPROVEMENT BUT NOT CHAMPION"
    else:
...
def build_report(summary_df, baseline_df, win_rates, verdict, qualitative_notes=None):
    ...
    report_lines = []
    report_lines.append("============================================================")
    report_lines.append("PHASE 9 TARGETED ROBUSTNESS REFINEMENT REPORT")
    report_lines.append("============================================================")
    report_lines.append("\nORIGINAL VALIDATION\n")
    report_lines.append(f"{'MODEL':<12}{'PSNR':<9}{'SSIM':<9}{'LPIPS':<10}{'MAE':<9}{'HF ERR'}")
    
    for model_key, label in [("phase5b", "Phase 5B"), ("phase7", "Phase 7"), ("phase8", "Phase 8"), ("phase9", "Phase 9")]:
    ...
    report_lines.append("\n------------------------------------------------------------\n")
    report_lines.append("PHASE 9 vs PHASE 8\n")
    for metric in METRICS:
        delta = p9_orig[metric] - p8_orig[metric]
        report_lines.append(f"{metric.upper():<7}: {delta:+.6f}")

    report_lines.append("\n------------------------------------------------------------\n")
    report_lines.append("STRESS TEST SUMMARY\n")
    report_lines.append(f"{'Condition':<25} | {'Phase 5B':<8} | {'Phase 7':<8} | {'Phase 8':<8} | {'Phase 9':<8} | Best")
    ...
    for condition in ["gaussian_noise", "gaussian_blur", "noise_plus_blur"]:
        for severity in ["low", "medium", "high"]:
            ...
            max_val = max(val_5b, val_7, val_8, val_9)
            if max_val == val_9:
                best_model = "Phase 9"
            elif max_val == val_8:
                best_model = "Phase 8"
            ...
            report_lines.append(f"{cond_name:<25} | {val_5b:<8.4f} | {val_7:<8.4f} | {val_8:<8.4f} | {val_9:<8.4f} | {best_model}")

    report_lines.append("\n------------------------------------------------------------\n")
    report_lines.append("PHASE 9 WIN RATES\n")
    
    # Phase 9 vs Phase 8
    report_lines.append("Phase 9 win rates against Phase 8:")
    ...
    # Phase 9 vs Phase 5B
    report_lines.append("\nPhase 9 win rates against Phase 5B:")
    for name, wins in win_rates.items():
        w = wins["p9_vs_p5b"]
        report_lines.append(
            f"  {name:<20}: PSNR {w['psnr']:.1f}%, SSIM {w['ssim']:.1f}%, LPIPS {w['lpips']:.1f}%, MAE {w['mae']:.1f}%, HF {w['hf_err']:.1f}%"
    ...
    # Phase 9 vs Phase 7
    report_lines.append("\nPhase 9 win rates against Phase 7:")
    for name, wins in win_rates.items():
        w = wins["p9_vs_p7"]
        report_lines.append(
            f"  {name:<20}: PSNR {w['psnr']:.1f}%, SSIM {w['ssim']:.1f}%, LPIPS {w['lpips']:.1f}%, MAE {w['mae']:.1f}%, HF {w['hf_err']:.1f}%"
    ...
    report_lines.append("Report whether Phase 9 introduces:\n")
    ...
    report_lines.append("\nCURRENT CHAMPION:")
    report_lines.append("Phase 9" if "CASE A" in verdict else "Phase 8")
    report_lines.append("\nREASON:")
    
    explanation = []
    # If delta is negligible and verdict is Case A
    if "CASE A" in verdict:
        explanation.append("Phase 9 successfully refinemented Phase 8. It preserved Phase 8's original fidelity")
        explanation.append(f"(clean PSNR is {p9_orig.psnr:.4f} vs Phase 8's {p8_orig.psnr:.4f}, a difference of only {p8_orig.psnr - p9_orig.psnr:.4f} dB)")
        explanation.append("while successfully transferring more of Phase 7's robustness under medium/high blur and noise+blur.")
        explanation.append("Therefore, Phase 9 is declared the NEW CHAMPION.")
    elif "CASE B" in verdict:
        explanation.append("Phase 9 improves blur robustness, but at the expense of a material regression on clean validation.")
        explanation.append(f"PSNR regressed from {p8_orig.psnr:.4f} to {p9_orig.psnr:.4f} ({p8_orig.psnr - p9_orig.psnr:.4f} dB regression).")
        explanation.append("Per the champion rules, Phase 8 is kept as champion.")
    elif "CASE D" in verdict:
        explanation.append("Phase 9 shows extremely strong robustness gains, making it a specialized robustness model,")
        explanation.append("but the clean validation regression is too high. Thus, Phase 8 remains the champion.")
    else:
        explanation.append("Phase 9 did not provide meaningful robustness improvements over Phase 8, or regressed")
        explanation.append("on both. Per the critical decision logic, Phase 8 remains the champion.")
...
def save_plots(summary_df, plots_dir):
    ...
    for metric in METRICS:
        ...
        for axis, condition in zip(axes, CONDITIONS):
            for model, color, label in (("phase5b", "#3568b8", "Phase 5B"), ("phase7", "#c45a11", "Phase 7"), ("phase8", "#9467bd", "Phase 8"), ("phase9", "#2ca02c", "Phase 9")):
...
def save_visuals(df, dataset, p4, p5b, p7, p8, p9, device, visual_root):
    scenarios = [
        ("gaussian_noise", "high"),
        ("gaussian_blur", "high"),
    ...
    for condition, severity in scenarios:
        ...
        pair["delta"] = pair["phase9"] - pair["phase8"]
        ...
        for label, index in chosen:
            ...
            titles = [
                ...
                "Phase 8 (Prev Champ)", "Phase 9 (Candidate)",
                "Phase 5B (Fid Champ)", "Phase 7 (Rob Spec)",
                "|Phase 8 - GT|", "|Phase 9 - GT|"
            ]
...
def main():
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for Phase 9 evaluation.")
    ...
    print(f"Loading Phase 9 model from: {P9_CKPT}")
    ...
    with torch.no_grad():
        p9_out = run_inference(p4, p9, inp)
    assert p9_out.shape == target.shape, f"Output shape mismatch: {p9_out.shape} vs {target.shape}"
    ...
    with torch.no_grad():
        for batch_number, batch in enumerate(loader):
            ...
            p9_orig = run_inference(p4, p9, original)
    ...
    all_records_df.to_csv(os.path.join(evaluation_dir, "phase9_metrics.csv"), index=False)
    ...
    with open(os.path.join(evaluation_dir, "phase9_summary.json"), "w", encoding="utf-8") as handle:
        json.dump(make_json_safe(summary), handle, indent=2)
    with open(os.path.join(evaluation_dir, "PHASE9_REPORT.txt"), "w", encoding="utf-8") as handle:
        handle.write(report)
    ...
    save_visuals(df, dataset, p4, p5b, p7, p8, p9, device, os.path.join(OUT_ROOT, "visual_comparisons"))
    
    print(f"\nPhase 9 evaluation complete. Verdict: {verdict}")
    print(report)
