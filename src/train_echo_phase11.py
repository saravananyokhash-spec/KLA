"""Phase 11 Degradation-Aware Restoration Script
...
def apply_kla_degradation(original_lr, target, sample_keys, epoch, cfg, device):
    ...
    p_clean = deg.get("preserve_original_probability", 0.35)
    p_g = deg.get("p_gaussian", 0.10)
    p_s = deg.get("p_speckle", 0.10)
    p_d = deg.get("p_downsample", 0.10)
    p_gs = deg.get("p_gaussian_speckle", 0.10)
    p_gd = deg.get("p_gaussian_downsample", 0.10)
    p_sd = deg.get("p_speckle_downsample", 0.10)
...
def main():
    parser = argparse.ArgumentParser(description="Phase 11 Degradation-Aware Restoration Training")
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA not available! Phase 11 training requires GPU execution.")
    ...
    print("=" * 60)
    print(f"PHASE 11 DEGRADATION-AWARE RESTORATION — EXPERIMENT: {cfg['experiment_name'].upper()}")
    ...
    student.load_state_dict(t9_chk["model_state_dict"])
    print("PASS")
    ...
    for epoch in range(1, epochs + 1):
        ...
        torch.save(ckpt_payload, os.path.join(checkpoints_dir, "echo_phase11_last.pth"))
        
        # Save best checkpoint
        if score > best_score:
            best_score = score
            torch.save(ckpt_payload, os.path.join(checkpoints_dir, "echo_phase11_best.pth"))
            print(f"  --> Saved new best checkpoint (Score: {best_score:+.4f})")

    elapsed = time.time() - start_time
    hist_df = pd.DataFrame(history)
    hist_csv_path = os.path.join(results_dir, "phase11_history.csv")
