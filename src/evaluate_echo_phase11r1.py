"""Phase 11R-1 Evaluation Script
Compares Phase 11R-1 against Phase 5B, Phase 7, Phase 8, Phase 9, Phase 10 (if available), and Phase 11.
...
P11R1_CKPT = "outputs/phase11r1_degradation_aware/checkpoints/echo_phase11r1_best.pth"

OUT_ROOT = "outputs/phase11r1_degradation_aware"
...
def add_records(records, condition, severity, sample_indices, p5b_out, p7_out, p8_out, p9_out, p10_out, p11_out, p11r1_out, target, lpips_model, decomposition):
...
def make_summary(df):
    ...
    for (condition, severity), group in df.groupby(["condition", "severity"], sort=False):
        ...
        rates = {
            "p11r1_vs_p9": {
                "psnr": float((pivot["psnr"]["phase11r1"] > pivot["psnr"]["phase9"]).mean() * 100),
                "ssim": float((pivot["ssim"]["phase11r1"] > pivot["ssim"]["phase9"]).mean() * 100),
                "lpips": float((pivot["lpips"]["phase11r1"] < pivot["lpips"]["phase9"]).mean() * 100),
                "mae": float((pivot["mae"]["phase11r1"] < pivot["mae"]["phase9"]).mean() * 100),
                "hf_err": float((pivot["hf_err"]["phase11r1"] < pivot["hf_err"]["phase9"]).mean() * 100),
            },
            "p11r1_vs_p11": {
                "psnr": float((pivot["psnr"]["phase11r1"] > pivot["psnr"]["phase11"]).mean() * 100),
                "ssim": float((pivot["ssim"]["phase11r1"] > pivot["ssim"]["phase11"]).mean() * 100),
                "lpips": float((pivot["lpips"]["phase11r1"] < pivot["lpips"]["phase11"]).mean() * 100),
                "mae": float((pivot["mae"]["phase11r1"] < pivot["mae"]["phase11"]).mean() * 100),
                "hf_err": float((pivot["hf_err"]["phase11r1"] < pivot["hf_err"]["phase11"]).mean() * 100),
            }
        }
...
def build_report(summary_df, baseline_df, win_rates, verdict, qualitative_notes=None):
    ...
    p11r1_orig = base.loc["phase11r1"]
    has_p10 = "phase10" in base.index
    p10_orig = base.loc["phase10"] if has_p10 else None

    report = []
    report.append("============================================================")
    report.append("PHASE 11R-1 DEGRADATION-AWARE RESTORATION REPORT")
    report.append("============================================================")
    report.append("\nORIGINAL VALIDATION\n")
    ...
    models.extend(["phase11", "phase11r1"])
    ...
    report.append("\n------------------------------------------------------------\n")
    report.append("PHASE 11R-1 vs PHASE 9 BASES DELTAS\n")
    for metric in METRICS:
        abs_delta = p11r1_orig[metric] - p9_orig[metric]
    ...
    report.append("\n------------------------------------------------------------\n")
    report.append("PHASE 11R-1 vs PHASE 11 BASES DELTAS\n")
    for metric in METRICS:
        abs_delta = p11r1_orig[metric] - p11_orig[metric]
        pct_delta = (abs_delta / (p11_orig[metric] + 1e-8)) * 100.0
        report.append(f"{metric.upper():<7}: Absolute: {abs_delta:+.6f} | Percentage: {pct_delta:+.4f}%")

    # Bilinear/Area Downsamplings represent OOD tests
    report.append("\n------------------------------------------------------------\n")
    report.append("ID vs OOD RESTORATION GAP FOR PHASE 11R-1\n")
    
    # ID: downsample medium (bicubic)
    id_m = summary_df[(summary_df.model == "phase11r1") & (summary_df.condition == "downsample") & (summary_df.severity == "medium")].iloc[0]
    # OOD: downsample low (bilinear) and high (area)
    ood_l = summary_df[(summary_df.model == "phase11r1") & (summary_df.condition == "downsample") & (summary_df.severity == "low")].iloc[0]
    ood_h = summary_df[(summary_df.model == "phase11r1") & (summary_df.condition == "downsample") & (summary_df.severity == "high")].iloc[0]
    ...
    for cond in CONDITIONS:
        for sev in SEVERITIES:
            ...
            p11r1_val = vals_dict.get("phase11r1", 0.0)
            ...
            row_str += f" | {p11_val:<6.2f} | {p11r1_val:<6.2f} | {best_model}"
            report.append(row_str)

    report.append("\n------------------------------------------------------------\n")
    report.append("PHASE 11R-1 WIN RATES AGAINST Phase 9 / Phase 11\n")
    for k, v in win_rates.items():
        w9 = v["p11r1_vs_p9"]
        w11 = v["p11r1_vs_p11"]
        report.append(f"  {k:<35} | vs P9: PSNR {w9['psnr']:.1f}%, SSIM {w9['ssim']:.1f}%, LPIPS {w9['lpips']:.1f}% | vs P11: PSNR {w11['psnr']:.1f}%, SSIM {w11['ssim']:.1f}%, LPIPS {w11['lpips']:.1f}%")
    ...
    report.append("\nCURRENT CHAMPION:")
    report.append("Phase 11R-1" if "CHAMPION" in verdict else "Phase 9")
    report.append("\nREASON:")
    if "NEW CHAMPION" in verdict:
        report.append("Phase 11R-1 successfully recovered clean-image validation fidelity to Phase 9 levels,")
        report.append("improved speckle noise restoration performance, and preserved Phase 11 robustness gains.")
    else:
        report.append("Phase 11R-1 failed to meet one or more decision gate constraints; Phase 9 remains champion.")
    report.append("============================================================")
...
def save_plots(summary_df, plots_dir, has_p10=True):
    ...
    for metric in METRICS:
        ...
        for idx, condition in enumerate(CONDITIONS):
            ...
            models.extend(["phase11", "phase11r1"])
            colors.extend(["#d62728", "#17becf"])
            labels_m.extend(["Phase 11", "Phase 11R-1"])
        ...
        axes_flat[0].legend()
        fig.suptitle(f"{labels[metric]} vs Degradation Severity (Phase 11R-1 vs Baselines)", y=1.02, fontsize=14)
        fig.tight_layout()
        fig.savefig(os.path.join(plots_dir, f"{metric}_vs_severity.png"), dpi=180, bbox_inches="tight")
        plt.close(fig)

def save_visuals(df, dataset, p4, p5b, p7, p8, p9, p11, p11r1, device, visual_root):
    ...
    for condition, severity in scenarios:
        ...
        pair["delta"] = pair["phase11r1"] - pair["phase9"]
        ...
        for label, index in chosen:
            ...
            with torch.no_grad():
                ...
                p11r1_out = run_inference(p4, p11r1, degraded, is_p10=False)
            ...
            imgs = [
                ...
                p11r1_out,
                (p9_out - target).abs(),
                (p11r1_out - target).abs()
            ]
            titles = [
                "Ground Truth", f"Degraded Input ({condition})", 
                "Phase 5B", "Phase 9 (Champ)",
                "Phase 11 (Baseline)", "Phase 11R-1 (Candidate)",
                "|Phase 9 - GT|", "|Phase 11R-1 - GT|"
            ]
            ...
            path = os.path.join(visual_root, f"{label}_cases" if label != "worst" else "worst_cases", f"{condition}_{severity}_{label}_sample_{index + 1:04d}.png")
            os.makedirs(os.path.dirname(path), exist_ok=True)
            fig.savefig(path, dpi=150, bbox_inches="tight")
            plt.close(fig)

def main():
    parser = argparse.ArgumentParser(description="Phase 11R-1 Evaluation")
    parser.add_argument("--batch-size", type=int, default=16, help="Batch size for evaluation")
    parser.add_argument("--max-samples", type=int, default=640, help="Limit number of validation samples evaluated")
    parser.add_argument("--sanity-only", action="store_true", help="Run quick 1-sample verification and exit")
    parser.add_argument("--qualitative-json", type=str, help="Path to qualitative inspection JSON notes")
    ...
    with torch.no_grad():
        out = run_inference(model_p4, p11r1, inp, is_p10=False)
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(loader):
            ...
            p11r1_orig = run_inference(model_p4, p11r1, original, is_p10=False)
            ...
            for cond in CONDITIONS:
                for sev in SEVERITIES:
                    ...
                    p11r1_out = run_inference(model_p4, p11r1, degraded, is_p10=False)
    ...
    eval_csv_path = os.path.join(OUT_ROOT, "results", "phase11r1_evaluation.csv")
    ...
    p11r1_base = base.loc["phase11r1"]
    
    psnr_diff_p9 = p11r1_base.psnr - p9_base.psnr
    ssim_diff_p9 = p11r1_base.ssim - p9_base.ssim
    
    # Decision Gate 1: Clean Validation Fidelity (Target: recovered PSNR & SSIM)
    orig_recovered = (psnr_diff_p9 >= -0.01) and (ssim_diff_p9 >= -0.0005)
    
    # Decision Gate 2: Speckle noise improvement vs Phase 11
    # Check speckle overall stress test averages
    p11r1_speckle = summary_df[(summary_df.model == "phase11r1") & (summary_df.condition == "speckle")]["psnr_mean"].mean()
    p11_speckle = summary_df[(summary_df.model == "phase11") & (summary_df.condition == "speckle")]["psnr_mean"].mean()
    speckle_improved = p11r1_speckle > p11_speckle
    
    # Decision Gate 3: Robustness Preservation (verify overall stress performance matches or exceeds P9/P11)
    p11r1_overall_stress = summary_df[summary_df.model == "phase11r1"]["psnr_mean"].mean()
    p9_overall_stress = summary_df[summary_df.model == "phase9"]["psnr_mean"].mean()
    
    if orig_recovered and speckle_improved and (p11r1_overall_stress > p9_overall_stress):
    ...
    # Save Report
    with open(os.path.join(OUT_ROOT, "results", "PHASE11R1_REPORT.txt"), "w", encoding="utf-8") as f:
        f.write(report)
    ...
    save_visuals(df, dataset, model_p4, p5b, p7, p8, p9, p11, p11r1, device, os.path.join(OUT_ROOT, "visual_comparisons"))
    
    print(f"\nPhase 11R-1 evaluation complete. Verdict: {verdict}")
    print(report)
