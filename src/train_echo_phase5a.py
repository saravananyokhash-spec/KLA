...
def main():
    config_path = "configs/phase5a.yaml"
    if not os.path.exists(config_path):
        config_path = "outputs/phase5a/configs/phase5a.yaml"
    ...
    with open(os.path.join(configs_dir, "phase5a.yaml"), "w", encoding="utf-8") as f:
        yaml.dump(cfg, f)

    l_weights = cfg["loss_weights"]

    print("=" * 60)
    print("PHASE 5A — LOSS REBALANCING FOR FAITHFUL RESTORATION")
    print(f"Device: {device}")
    print(f"Output Directory: {out_dir}")
    print("-" * 60)
    print("Phase 5A Loss Configuration")
    print("----------------------------")
    ...
    print(f"Perceptual (LPIPS)  : {l_weights.get('lpips', 0.005)}")
    print(f"P4 Anchor           : {l_weights.get('p4_anchor', 0.02)}")
    print("=" * 60)

    # --- MANDATORY SANITY CHECKS ---
    print("\n" + "=" * 50)
    print("RUNNING PHASE 5A SANITY CHECKS")
    print("=" * 50)

    # Check 1: CUDA
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is not available! Phase 5A training requires GPU.")
    ...
    # Instantiate Phase 5 Model Architecture for Phase 5A
    ...
    # Check 8: Phase 5A trainable
    p5a_trainable = any(p.requires_grad for p in model_p5a.parameters())
    if not p5a_trainable:
        raise ValueError("CRITICAL ERROR: Model parameters are NOT trainable!")
    print("Sanity Check 8: Phase 5A model parameters trainable: PASSED")
    ...
    s_p5a_hr, s_xlf, s_xmf, s_xhf, s_feat = model_p5a(s_lr_up, s_p4_hr)

    # Check 9: Output shape
    if list(s_p5a_hr.shape) != [s_in.shape[0], 1, 256, 256]:
        raise ValueError(f"Output shape error! Got {s_p5a_hr.shape}")
    print(f"Sanity Check 9: Output shape {list(s_p5a_hr.shape)}: PASSED")

    # Check 10: Finiteness
    if not torch.isfinite(s_p5a_hr).all():
        raise ValueError("Output contains NaNs/Infs!")
    print("Sanity Check 10: Output values finite: PASSED")

    # Check 11: Output Range
    if s_p5a_hr.min() < 0.0 or s_p5a_hr.max() > 1.0:
        raise ValueError(f"Output range exceeded! min={s_p5a_hr.min():.4f}, max={s_p5a_hr.max():.4f}")
    ...
    l_pixel = F.l1_loss(s_p5a_hr, s_tgt)
    l_ssim = 1.0 - ssim_pytorch(s_p5a_hr, s_tgt)
    l_fft = F.l1_loss(torch.fft.rfft2(s_p5a_hr, norm="ortho"), torch.fft.rfft2(s_tgt, norm="ortho"))
    
    tgt_lf, tgt_mf, tgt_hf = decomp_helper(s_tgt)
    pred_lf, pred_mf, pred_hf = decomp_helper(s_p5a_hr)
    
    l_hf_direct = F.l1_loss(pred_hf, tgt_hf)
    l_hf_comp = F.l1_loss(s_xhf, tgt_hf)
    l_edge = F.l1_loss(sobel_filter(s_p5a_hr), sobel_filter(s_tgt))
    l_lpips = ssim_lpips_differentiable(s_p5a_hr, s_tgt, lpips_model)
    l_anchor = F.l1_loss(s_p5a_hr, s_p4_hr)
    ...
    l_total = (l_weights.get("pixel_l1", 2.0) * l_pixel +
               ...
               l_weights.get("lpips", 0.005) * l_lpips +
    ...
    optimizer = torch.optim.AdamW(model_p5a.parameters(), lr=cfg["training"]["lr"])
    optimizer.zero_grad()
    l_total.backward()

    # Check 15: Phase 5A receives gradients
    has_grads = any(p.grad is not None and p.grad.norm() > 0 for p in model_p5a.parameters())
    if not has_grads:
        raise ValueError("Phase 5A model received NO gradients!")
    print("Sanity Check 15: Phase 5A model receives gradients: PASSED")
    ...
    grad_norm = torch.nn.utils.clip_grad_norm_(model_p5a.parameters(), 1.0)
    ...
    freq_grads = any(p.grad is not None and p.grad.norm() > 0 for p in model_p5a.freq_branch.parameters())
    if not freq_grads:
    ...
    spatial_grads = any(p.grad is not None and p.grad.norm() > 0 for p in model_p5a.spatial_branch.parameters())
    ...
    fusion_grads = any(p.grad is not None and p.grad.norm() > 0 for p in model_p5a.fusion.parameters())
    if not fusion_grads:
    ...
    o_optimizer = torch.optim.AdamW(model_p5a.parameters(), lr=1.0e-3)
    ...
    for step in range(50):
        ...
        o_p5a_hr, o_xlf, o_xmf, o_xhf, _ = model_p5a(o_lr_up, o_p4_hr)

        l_pix = F.l1_loss(o_p5a_hr, o_tgt)
        l_ss = 1.0 - ssim_pytorch(o_p5a_hr, o_tgt)
        l_ff = F.l1_loss(torch.fft.rfft2(o_p5a_hr, norm="ortho"), torch.fft.rfft2(o_tgt, norm="ortho"))
        o_pred_lf, o_pred_mf, o_pred_hf = decomp_helper(o_p5a_hr)
        l_hfd = F.l1_loss(o_pred_hf, o_tgt_hf)
        l_edg = F.l1_loss(sobel_filter(o_p5a_hr), sobel_filter(o_tgt))
    ...
    # Reset Model for Full Training Run
    model_p5a = SpatialFrequencyRestorationNet(
        spatial_channels=cfg["model"]["spatial_channels"],
        freq_channels=cfg["model"]["freq_channels"],
        fusion_channels=cfg["model"]["fusion_channels"],
    ...
    optimizer = torch.optim.AdamW(model_p5a.parameters(), lr=cfg["training"]["lr"], weight_decay=cfg["training"]["weight_decay"])
    scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer, gamma=cfg["training"]["scheduler_gamma"])

    # --- 5-EPOCH PHASE 5A TRAINING RUN ---
    epochs = cfg["training"]["epochs"]
    print("\n" + "=" * 50)
    print(f"STARTING PHASE 5A ({epochs} EPOCHS) TRAINING RUN")
    print("=" * 50)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch in train_loader:
            ...
            optimizer.zero_grad()
            b_p5a_hr, b_xlf, b_xmf, b_xhf, _ = model_p5a(b_lr_up, b_p4_hr)
            b_pred_lf, b_pred_mf, b_pred_hf = decomp_helper(b_p5a_hr)

            l_pixel = F.l1_loss(b_p5a_hr, b_tgt)
            l_ssim = 1.0 - ssim_pytorch(b_p5a_hr, b_tgt)
            l_fft = F.l1_loss(torch.fft.rfft2(b_p5a_hr, norm="ortho"), torch.fft.rfft2(b_tgt, norm="ortho"))
            l_hf_direct = F.l1_loss(b_pred_hf, b_tgt_hf)
            l_hf_comp = F.l1_loss(b_xhf, b_tgt_hf)
            l_edge = F.l1_loss(sobel_filter(b_p5a_hr), sobel_filter(b_tgt))
            l_lpips = ssim_lpips_differentiable(b_p5a_hr, b_tgt, lpips_model)
            l_anchor = F.l1_loss(b_p5a_hr, b_p4_hr)
            ...
            loss.backward()
            torch.nn.utils.clip_grad_norm_(model_p5a.parameters(), 1.0)
            optimizer.step()
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                v_p5a_hr, v_xlf, v_xmf, v_xhf, _ = model_p5a(v_lr_up, v_p4_hr)
                v_pred_lf, v_pred_mf, v_pred_hf = decomp_helper(v_p5a_hr)

                val_psnr_list.append(calculate_psnr(v_p5a_hr, v_tgt))
                val_ssim_list.append(ssim_pytorch(v_p5a_hr, v_tgt).item())
                val_lpips_list.append(ssim_lpips_differentiable(v_p5a_hr, v_tgt, lpips_model).item())
                val_mae_list.append(F.l1_loss(v_p5a_hr, v_tgt).item())
                val_edge_list.append(F.l1_loss(sobel_filter(v_p5a_hr), sobel_filter(v_tgt)).item())
        ...
        # Metric Score for Model Checkpointing (prioritizing PSNR recovery and HF error reduction)
        ...
        print(f"Epoch {epoch:02d}/{epochs:02d} | Train Loss: {avg_train_loss:.4f} | Val PSNR: {m_psnr:.4f} dB | Val SSIM: {m_ssim:.4f} | Val LPIPS: {m_lpips:.4f} | Val HF Err: {m_hf_err:.4f} | Score: {score:+.4f}")

        # Save Checkpoint
        ckpt_payload = {
            "epoch": epoch,
            "model_state_dict": model_p5a.state_dict(),
        ...
        torch.save(ckpt_payload, os.path.join(checkpoints_dir, "echo_phase5a_last.pth"))

        if score > best_score:
            best_score = score
            torch.save(ckpt_payload, os.path.join(checkpoints_dir, "echo_phase5a_best.pth"))
            print(f"  --> Saved new best checkpoint (Score: {best_score:+.4f})")

    elapsed = time.time() - start_time
    print(f"\nPhase 5A training finished in {elapsed/60.0:.2f} mins.")

    # Save History CSV
    hist_df = pd.DataFrame(history)
    hist_csv_path = os.path.join(results_dir, "phase5a_history.csv")
