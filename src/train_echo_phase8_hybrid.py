...
def main():
    parser = argparse.ArgumentParser(description="Phase 8 Hybrid Fine-Tuning")
    ...
    print("=" * 60)
    print(f"PHASE 8 HYBRID FINE-TUNE — EXPERIMENT: {cfg['experiment_name'].upper()}")
    ...
    # [4] Dataset paths valid
    dataset_root = cfg["dataset"]["dataset_root"]
    train_csv = cfg["dataset"]["train_csv"]
    val_csv = cfg["dataset"]["val_csv"]
    print(f"Check [4] Dataset paths valid: Root='{dataset_root}', TrainCSV='{train_csv}', ValCSV='{val_csv}'...", end=" ")
    ...
    # [5] Training dataset loads
    print("Check [5] Training dataset loads...", end=" ")
    train_dataset = KLADataset(dataset_root=dataset_root, split="train", csv_path=train_csv)
    print(f"PASS (Length: {len(train_dataset)})")

    # [6] Validation dataset loads
    print("Check [6] Validation dataset loads...", end=" ")
    ...
    # Instantiate Fine-tuning model (student) warm-started from Phase 5B
    print(f"Initializing Phase 8 student model from Phase 5B checkpoint: {p5b_ckpt_path}")
    ...
    student.load_state_dict(t5b_chk["model_state_dict"])
    ...
    # [7] Tensor dimensions correct
    print(f"Check [7] Tensor dimensions correct...", end=" ")
    ...
    # [8] Model forward pass works
    print("Check [8] Model forward pass works...", end=" ")
    with torch.no_grad():
    ...
    # [9] Teacher forward passes work
    print("Check [9] Teacher forward passes work...", end=" ")
    with torch.no_grad():
        s_t5b_hr, _, _, _, _ = teacher_5b(s_lr_up, s_p4_hr)
        s_t7_hr, _, _, _, _ = teacher_7(s_lr_up, s_p4_hr)
    print("PASS")

    # [10] Loss values are finite
    print("Check [10] Loss values are finite...", end=" ")
    with torch.no_grad():
    ...
    # [11] No NaN / Inf
    print("Check [11] No NaN / Inf...", end=" ")
    if not torch.isfinite(s_pred_hr).all():
        raise ValueError("Student predictions contain NaNs or Infs!")
    print("PASS")

    # [12] Augmentation distribution check
    print("Check [12] Augmentation distribution check...", end=" ")
    # Let's count proportions on a larger sample to verify 60/20/20 target distribution approximately
    ...
    print(f"PASS (Proportions in sample: clean={orig_pct*100:.1f}%, blur={blur_pct*100:.1f}%, noise+blur={nb_pct*100:.1f}%)")

    # [13] 2-sample overfit test
    print("Check [13] Running 2-sample overfit test...", end=" ")
    ...
    # Restore student model to clean weights
    student.load_state_dict(t5b_chk["model_state_dict"])

    # [14] Checkpoint save/load test
    print("Check [14] Checkpoint save/load test...", end=" ")
    ...
    print("\n" + "=" * 50)
    print(f"STARTING {epochs} EPOCHS HYBRID FINE-TUNING RUN")
    print("=" * 50)
    ...
    for epoch in range(1, epochs + 1):
        ...
        # Scoring system prioritizing original PSNR/SSIM and LPIPS preservation (same as Phase 7)
        ...
        torch.save(ckpt_payload, os.path.join(checkpoints_dir, "echo_phase8_hybrid_last.pth"))
        
        # Save best checkpoint
        if score > best_score:
            best_score = score
            torch.save(ckpt_payload, os.path.join(checkpoints_dir, "echo_phase8_hybrid_best.pth"))
            print(f"  --> Saved new best checkpoint (Score: {best_score:+.4f})")

    elapsed = time.time() - start_time
    hist_df = pd.DataFrame(history)
    hist_csv_path = os.path.join(results_dir, "phase8_hybrid_history.csv")
